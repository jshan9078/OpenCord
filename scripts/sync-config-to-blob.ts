...
function buildGithubPolicyBlock(defaultLogin?: string): string {
  ...
  if (defaultLogin) {
    lines.push(
      `- Default authenticated GitHub login for this deployment: ${defaultLogin}.`,
    );
  }
...
const DISCORD_OUTPUT_POLICY_BLOCK = [
  "## CRITICAL: DISCORD MESSAGING GUARDRAILS",
  "",
...
function upsertAgentsPolicies(content: string, defaultLogin?: string): string {
  const withGithub = upsertSection(
    content,
    "GitHub CLI Policy",
    buildGithubPolicyBlock(defaultLogin),
  );
  return upsertSection(
    withGithub,
    "Discord Message Rendering",
    DISCORD_OUTPUT_POLICY_BLOCK,
  );
}
...
async function main(): Promise<void> {
  ...
  if (!token) {
    throw new Error(
      "BLOB_READ_WRITE_TOKEN is required. Run `vercel env pull` first.",
    );
  }
